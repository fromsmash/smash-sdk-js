...
export class Discovery extends Client {
    ...
    constructor(params: ClientParameters) {
        super({ service: "discovery", ...params });
    }

    listPublicServices(params: ListPublicServicesInput): Promise<ListPublicServicesOutput> {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const response = await this.client.handle<ListPublicServicesResponse>(request);
